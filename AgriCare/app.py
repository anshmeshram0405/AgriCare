...
@ app.route('/Scheme')
def schema():
    title = 'AgriCare - Scheme'
    return render_template('Schema.html', title=title)

@ app.route('/Bank-Scheme')
def Bank_scheme():
    title = 'AgriCare - Bank-Scheme'
    return render_template('Bank-Scheme.html', title=title)

@app.route('/About_us')
def about_us():
    title ='AgriCare - About Us'
    return render_template("About_us.html", title=title)
    
@app.route('/contact')
def contact():
    title ='AgriCare - Contact Us'
...
disease_model = ResNet9(3, len(disease_classes))            # Neural network model
...
def predict_image(img, model=disease_model):                # input parameters
    """
    ...
    image = Image.open(io.BytesIO(img))                     # reads the input image data
    img_t = transform(image)                                # converting it into a tensor 
...
def disease_prediction():
    title = 'AgriCare - Disease Detection'
...
crop=pd.read_csv("final_dataset.csv").copy()


@app.route('/crop')
def crop_recommend():
    states=sorted(crop['State_Name'].unique())
    season = crop['Season'].unique()
    crop["District_Name"] = crop["State_Name"] + "_" + crop["District_Name"]
    districts = sorted(crop['District_Name'].unique())

    # states.insert(0, 'Select States')
    # season.insert(0, 'Select Season')

    return render_template('crop.html',states=states,districts=districts,seasons=season)
...
def predict():
    ...
    district_name=request.form.get('districts')
    season=request.form.get('season').strip()
    area=float(request.form.get('area'))
    pH=float(request.form.get('ph'))
    raifnall=float(request.form.get('rainfall'))
    temperature=float(request.form.get('temperature'))
    district_name=district_name.split('_')[1]
    output=model.predict(pd.DataFrame([[state_name, district_name, season, area, temperature, pH, raifnall]],
...
def fertilizer_recommendation():
    title = 'AgriCare - Fertilizer Suggestion'
...
app.config['CORS_HEADERS'] = 'Content-Type'                         # CORS (Cross-Origin Resource Sharing) headers
...
def index():
    ...
    return render_template('Cost.html', context=context)
